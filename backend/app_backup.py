...
from db_new import db_new as db  # Use new database as primary
...
# NEW API ENDPOINTS USING REDESIGNED DATABASE SCHEMA
# These provide complete rack data including annotations, comments, ratings

@app.route('/api/v2/racks/<rack_id>', methods=['GET'])
def get_rack_complete_v2(rack_id):
    """Get complete rack data including annotations, comments, ratings (v2 schema)"""
    try:
        if not rack_id or rack_id in ['undefined', 'null', 'None']:
            return jsonify({'error': 'Invalid rack ID provided'}), 400
        ...
        # Connect to new database if not connected
        if not db.connected:
            if not db.connect():
                return jsonify({'error': 'Database connection failed'}), 500
        
        rack = db.get_rack_with_full_data(rack_id)
        if rack:
            # Filter unused macros
            if 'analysis' in rack and 'macro_controls' in rack['analysis']:
                used_macros = []
                for macro in rack['analysis']['macro_controls']:
                    # Only include macros that have been assigned/mapped or have non-default values
                    if (macro.get('mapped', False) or 
                        macro.get('value', 0) != 0 or 
                        macro.get('name', '').strip() != '' and macro.get('name') != f"Macro {macro.get('id', '')}"):
                        used_macros.append(macro)
                rack['analysis']['macro_controls'] = used_macros
        ...
        logger.error(f"Error in get_rack_complete_v2 for rack_id '{rack_id}': {str(e)}")
        return jsonify({'error': f'Failed to get rack: {str(e)}'}), 500

@app.route('/api/v2/racks/<rack_id>/annotations', methods=['POST'])
@token_required
def add_annotation_v2(current_user, rack_id):
    """Add annotation to a rack (v2 schema)"""
    try:
        if not db.connected and not db.connect():
...
@app.route('/api/v2/racks/<rack_id>/comments', methods=['POST'])
@token_required
def add_comment_v2(current_user, rack_id):
    """Add comment to a rack (v2 schema)"""
    try:
        if not db.connected and not db.connect():
            return jsonify({'error': 'Database not available'}), 500
...
@app.route('/api/v2/racks/<rack_id>/rate', methods=['POST'])
@token_required
def rate_rack_v2(current_user, rack_id):
    """Rate a rack (v2 schema)"""
    try:
        if not db.connected and not db.connect():
            return jsonify({'error': 'Database not available'}), 500
        ...
        logger.error(f"Error rating rack: {str(e)}")
        return jsonify({'error': f'Failed to rate rack: {str(e)}'}), 500

# New Enhanced Endpoints using redesigned database
@app.route('/api/racks/<rack_id>/enhanced', methods=['GET'])
def get_rack_enhanced(rack_id):
    """Get complete rack data with embedded annotations, comments, ratings"""
    try:
        # Get rack with all embedded data in single query
        rack = db.get_rack_with_full_data(rack_id)
        ...
        logger.error(f"Error getting enhanced rack: {str(e)}")
        return jsonify({'error': f'Failed to get rack: {str(e)}'}), 500

@app.route('/api/racks/<rack_id>/annotate', methods=['POST'])
@token_required
def add_rack_annotation(current_user, rack_id):
    """Add annotation to rack (embedded in rack document)"""
    try:
        data = request.get_json()
        if not data or 'content' not in data:
            return jsonify({'error': 'Annotation content is required'}), 400
...
@app.route('/api/racks/<rack_id>/comment', methods=['POST'])
@token_required
def add_rack_comment(current_user, rack_id):
    """Add comment to rack (embedded in rack document)"""
    try:
        data = request.get_json()
        if not data or 'content' not in data:
            return jsonify({'error': 'Comment content is required'}), 400
...
@app.route('/api/racks/<rack_id>/rate', methods=['POST'])
@token_required
def rate_rack_enhanced(current_user, rack_id):
    """Rate rack (embedded in rack document)"""
    try:
        data = request.get_json()
        if not data or 'rating' not in data:
            return jsonify({'error': 'Rating is required'}), 400
...
def migrate_database():
    """Migrate from old schema to new schema (admin only)"""
    # This should be protected and only run once
    try:
